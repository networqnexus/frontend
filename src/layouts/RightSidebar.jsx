  ...
  return (
    <div className="flex flex-col gap-4 min-w-0">
      ...
      {suggestions.length > 0 && (
        <div className="rounded-xl border border-border bg-card p-4 lg:p-3 xl:p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm lg:text-xs xl:text-sm font-semibold text-foreground shrink-0">People you may know</h3>
            <Button variant="ghost" size="xs" className="shrink-0 text-xs" onClick={() => navigate("/network")}>
              See all <ChevronRight size={12} />
            </Button>
          </div>
          <div className="flex flex-col gap-3 lg:gap-2 xl:gap-3">
            {suggestions.map(u => (
              <div key={u._id} className="flex items-center gap-3 lg:gap-2 xl:gap-3">
                <div
                  className="w-9 h-9 lg:w-7 lg:h-7 xl:w-9 xl:h-9 rounded-full bg-primary/10 flex items-center justify-center text-xs font-bold text-primary shrink-0 overflow-hidden cursor-pointer"
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/profile/${u.username}`)}>
                  <p className="text-sm lg:text-xs xl:text-sm font-medium text-foreground truncate leading-tight hover:text-primary transition-colors">{u.name}</p>
