...
import { Search, Send, Phone, Video, MoreHorizontal, Check, CheckCheck, X, Loader2, EyeOff } from "lucide-react";
...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    socket.on("receive_message", (msg) => {
      ...
      const isActiveSender = activeConvRef.current?.user._id === senderId;
      if (isActiveSender) {
        setMessages(prev => [...prev, msg]);
      }
      setConversations(prev => prev.map(c =>
        c.user._id === senderId
          ? { ...c, lastMessage: msg, unread: isActiveSender ? 0 : (c.unread || 0) + 1 }
          : c
      ));
    });

    socket.on("typing", ({ senderId }) => {
      if (activeConvRef.current?.user._id === senderId) setTyping(true);
    });
    socket.on("stop_typing", ({ senderId }) => {
      if (activeConvRef.current?.user._id === senderId) setTyping(false);
    });
  ...
  const handleTyping = (e) => {
    ...
    socket.emit("typing", { receiverId: activeConv.user._id, senderId: user?.id });
  ...
  const getStatus = (convUser) => {
    if (!convUser) return null;
    if (convUser.hideOnlineStatus) return { text: null, dot: false, hidden: true };
    const isOnline = onlineUsers.includes(convUser._id);
    if (isOnline) return { text: "Active now", dot: true, hidden: false };
    const ls = formatLastSeen(convUser.lastSeen);
    return { text: ls, dot: false, hidden: false };
  };
        ...
        <div className={`w-full lg:w-72 xl:w-80 shrink-0 border-r border-border flex flex-col ${mobileView === "chat" ? "hidden lg:flex" : "flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              const status = getStatus(conv.user);
              return (
                <button key={conv.user._id} onClick={() => handleSelectConv(conv)}
                  ...
                  <div className="relative shrink-0">
                    ...
                    {status?.dot && <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 rounded-full ring-2 ring-card" />}
                  </div>
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                      {status?.hidden
                        ? <><EyeOff size={9} className="opacity-50" /><span className="opacity-50">Status hidden</span></>
                        : status?.dot
                          ? <span className="text-emerald-500 font-medium">Active now</span>
                          : status?.text || conv.user?.headline}
                    </p>
            ...
            <>
              ...
              <div className="flex items-center gap-3 px-4 py-3 border-b border-border shrink-0">
                ...
                <div className="relative shrink-0">
                  ...
                  {getStatus(activeConv.user)?.dot && <span className="absolute bottom-0 right-0 w-2 h-2 bg-emerald-500 rounded-full ring-2 ring-card" />}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    {typing
                      ...
                      : (() => {
                          const s = getStatus(activeConv.user);
                          if (s?.hidden) return <><EyeOff size={9} className="opacity-40" /><span className="opacity-40">Status hidden</span></>;
                          if (s?.dot) return <span className="text-emerald-500 font-medium">Active now</span>;
                          return s?.text || activeConv.user?.headline || "";
                        })()
                    }
                  </p>
                </div>
